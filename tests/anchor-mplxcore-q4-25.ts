...
describe("anchor-mplxcore-q4-25", () => {
  ...
  // const umi = console.log(
  //   `payer / system_wallet ${payer.publicKey.toString()}`,
  // );
  ...
  describe("UpdateNft", () => {
    it("Update an NFT", async () => {
      let assetAccount = await fetchAsset(umi, asset.publicKey.toBase58());
      assert.equal(assetAccount.name, "Test NFT");
      ...
      await program.methods
        ...
        assetAccount = await fetchAsset(umi, asset.publicKey.toBase58());
      assert.equal(assetAccount.name, newName);
    });
